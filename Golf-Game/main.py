...
restartButton = [winwidth / 2, 20, 30]
restartButtonColor = (128, 128, 128)  # Grey color
...
# Button Drawing
    ...
    pygame.draw.circle(win, restartButtonColor, (restartButton[0], restartButton[1]), restartButton[2])
    font = pygame.font.SysFont('comicsansms', 20)
    text = font.render('R', 1, (255, 255, 255))
    win.blit(text, (restartButton[0] - text.get_width() / 2, restartButton[1] - text.get_height() / 2))
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if ((pos[0] - restartButton[0]) ** 2 + (pos[1] - restartButton[1]) ** 2) ** 0.5 < restartButton[2]:
                level = 1
                setup(level)
                list = courses.getPar(1)
                par = list[level - 1]
